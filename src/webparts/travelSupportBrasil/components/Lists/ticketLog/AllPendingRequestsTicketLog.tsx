...
export default function AllPendingRequestsTicketLog() {
  const { allRequestsTicketLog } = useContext(Context);
  const [requestDetails, setRequestDetails] = useState({...allRequestsTicketLog[0], open:false});
  const [filter, setFilter] = useState({
    macroprocesso: '',
    processo: ''
  });
  const [solicitacoesFiltradas, setSolicitacoesFiltradas] = useState<IRequests_AllFields[]>(
    allRequestsTicketLog.filter( (req:IRequests_AllFields) => req.STATUS_APROVACAO === "Aprovado" && ( req.STATUS !== "Sucesso" && req.STATUS !== "Rejeitado"))
    );
  ...
  useEffect(()=>setSolicitacoesFiltradas(allRequestsTicketLog.filter( (req:IRequests_AllFields) => req.STATUS_APROVACAO === "Aprovado" && ( req.STATUS !== "Sucesso" && req.STATUS !== "Rejeitado"))), [allRequestsTicketLog]);

  useEffect(()=>setSolicitacoesFiltradas(allRequestsTicketLog
  ...
  const uniqueMacroprocesso = unique(allRequestsTicketLog.map(row => row.MACROPROCESSO));
