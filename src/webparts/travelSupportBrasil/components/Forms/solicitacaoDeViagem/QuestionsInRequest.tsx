...
const schema: yup.ObjectSchema<IRequests_AllFields> = yup.object().shape({
  ...
  BENEFICIARIO_EMAIL: yup.string()
    .email()
    .matches(/@vale|@itv|@ctss|@newsteel/)
    .required(),
  BENEFICIARIO_EMPRESA_COD: yup.string(),
  BENEFICIARIO_EMPRESA_NOME: yup.string(),

  TIPO_SERVICO_VIAGEM: yup.string()
    .required(),
  MOTIVO: yup.string()
    .min(50)
    .required()
...
export default function QuestionsInRequest() {
        ...
        <Grid container spacing={3} justify="space-between">
          ...
          <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
            ...
            <Controller
              as={
                <Select disabled fullWidth>
                  <MenuItem value="Dúvida na solicitação de viagem">Dúvida na solicitação de viagem</MenuItem>
                </Select>
              }
              id="Process"
              name="PROCESSO"
              defaultValue="Dúvida na solicitação de viagem"
              control={control}
          ...
          <Grid item xs={12} sm={8} md={8} lg={8} xl={8}>
            <FormControl
              component="fieldset"
              error={errors.TIPO_SERVICO_VIAGEM?true:false}>
            <FormLabel component="legend">Assunto</FormLabel>
            <RadioGroup
              aria-label="TIPO_SERVICO_VIAGEM"
              name="TIPO_SERVICO_VIAGEM"
              row
            >
              <FormControlLabel
                value="Passagem aérea"
