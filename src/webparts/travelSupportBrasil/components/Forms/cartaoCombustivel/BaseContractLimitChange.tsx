...
const schema: yup.ObjectSchema<IRequests_AllFields> = yup.object().shape({
  ...
  LIMITE_ATUAL: yup.number().required(),
  NOVO_LIMITE: yup.number().required(),
  MOTIVO: yup.string().required(),
});

export default function BaseContractLimitChange(){
          ...
          <Grid container spacing={3} justify="space-between">
            ...
            <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
              ...
              <Controller
                as={
                  <Select disabled fullWidth>
                    <MenuItem value="Alteração de limite do contrato da base">Alteração de limite do contrato da base</MenuItem>
                  </Select>
                }
                id="Process"
                name="PROCESSO"
                defaultValue="Alteração de limite do contrato da base"
                control={control}
              />
            </Grid>
            ...
            <Grid item xs={12} sm={6} md={6} lg={6} xl={6} >
              <TextField type="number" name="LIMITE_ATUAL"
                label="Limite atual" variant="outlined"
                InputLabelProps={{ shrink: true }}
                inputRef={register}
                inputProps={{ min: 1 }}
                error={errors.LIMITE_ATUAL?true:false}
                helperText={errors.LIMITE_ATUAL && errors.LIMITE_ATUAL.message}
              />
            </Grid>

            <Grid item xs={12} sm={6} md={6} lg={6} xl={6} >
              <TextField type="number" name="NOVO_LIMITE"
                label="Novo limite" variant="outlined"
                InputLabelProps={{ shrink: true }}
                inputRef={register}
                inputProps={{ min: 1 }}
            ...
            <Grid item xs={12} sm={12} md={12} lg={12} xl={12} >
              <TextField
                ...
                rows={5}
                name="MOTIVO"
                label="Motivo para alteração do limite"
                inputRef={register}
